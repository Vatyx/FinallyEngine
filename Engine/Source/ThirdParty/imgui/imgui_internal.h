...
#define ImFloorStd(X)                                                                                                                                \
    floorf(X)  // We already uses our own ImFloor() { return (float)(int)v } internally so the standard one wrapper is named differently (it's used by
               // e.g. stb_truetype)
...
enum ImGuiSelectableFlagsPrivate_
{
    ...
    ImGuiSelectableFlags_SpanAvailWidth = 1 << 23,   // Span all avail width even if we declared less for layout purpose. FIXME: We may be able to
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId = 1 << 4,  // Allow scoring and considering the current NavId as a move target candidate. This is used when
                                                   // the move source is offset (e.g. pressing PageDown actually needs to send a Up move request, if
                                                   // we are pressing PageDown from the bottom-most item we need to stay in place)
...
struct ImGuiPopupData
{
    ...
    ImGuiID OpenParentId;  // Set on OpenPopup(), we need this to differentiate multiple menu sets from each others (e.g. inside menu bar vs loose
                           // menu items)
...
struct ImGuiContext
{
    ...
    bool HoveredIdDisabled;  // At least one widget passed the rect test, but has been discarded by disabled flag or popup inhibit. May be true even
                             // if HoveredId == 0.
    ...
    ImU64 ActiveIdUsingKeyInputMask;  // Active widget will want to read those key inputs. When we grow the ImGuiKey enum we'll need to either to
                                      // order the enum to make useful keys come first, either redesign this into e.g. a small array.
    ...
    ImGuiID NavFocusScopeId;    // Identify a selection scope (selection code often wants to "clear other items" when landing on an item of the
                                // selection set)
    ...
    ImRect NavScoringRect;  // Rectangle used for scoring, in screen space. Based of window->DC.NavRefRectRel[], modified for directional navigation
                            // scoring.
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec1 ColumnsOffset;  // Offset to the current column (if ColumnsCurrent > 0). FIXME: This and the above should be a stack to allow use cases
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImRect ContentRegionRect;  // FIXME: This is currently confusing/misleading. It is essentially WorkRect but not handling of scrolling. We
                               // currently rely on it as right/bottom aligned sizing operation need some size to rely on.
...
IMGUI_API bool IsItemToggledSelection();  // Was the last item selection toggled? (after Selectable(), TreeNode() etc. We only returns toggle _event_
...
IMGUI_API bool TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label,
...
// silence Clang warnings (see #2036). e.g. " extern template IMGUI_API float RoundScalarWithFormatT<float, float>(const char* format, ImGuiDataType
// data_type, float v); "
...
IMGUI_API void ImFontAtlasBuildRender1bppRectFromString(ImFontAtlas* atlas, int atlas_x, int atlas_y, int w, int h, const char* in_str,
